...
TexturedMesh::TexturedMesh(TexturedMesh &mesh) : TriangleMesh(mesh)
{
    ...
    {
    	Material* old_mat = mesh.m_materials[i];
    	...
    	new_mat->m_texture = new Texture(*(old_mat->m_texture));
    ...
    {
    	MaterialFaceList* new_matList = new MaterialFaceList;
    	MaterialFaceList* old_matList = *it;
    ...
    m_textureCoords = new float[2 * m_numVertices];
