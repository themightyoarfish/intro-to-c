...
Camera MainWindow::m_cam;
    ...
    MainWindow::MainWindow(string filename, string mapName, string start, string end)
{
	m_logger << "Creating main window." << endl;

	// Save pointer to current instance. Later on we
	// will create a proper singleton here...
	MainWindow::master = this;
	
	cout << mapName << " " << start << " " << end << endl;
	...
	m_logger << "Creating model" << endl;

	// Create a fighter instance
 	m_fighter = new Fighter();
 	Read3DS reader(filename);
 	reader.getMesh(*m_fighter);
	...
	m_logger << "Loading skybox" << endl;
	m_skybox = new Skybox(2048, names);

	unsigned found = filename.find_last_of("/\\");
	string basePath = filename.substr(0, found+1);

	m_logger << "Creating asteroid field" << endl;
	m_asteroidField = new AsteroidField(30, basePath);
    
    // Calculate path
    PathPlanner searchpath(mapName);
    list<Vertex<float> >::iterator it;
    m_path = searchpath.getPath(m_fighter->getPosition(), start, end);
  
   
    // Render path
    m_navRoute = new asteroids::NavigationRoute(m_path);
    
	// Call main loop
	m_logger << "Calling main loop" << endl;
    glutMainLoop();
}
...
MainWindow::~MainWindow()
{
	...
	m_logger << "Main window closed" << endl;
}
...
void MainWindow::keyPressed(unsigned char key, int x, int y)
{
	...
	// let the fighter shoot if space is pressed
 	if (key == ' ')
 	{
 	    m_fighter->shoot();
 	}
